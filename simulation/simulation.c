...
int main(){
    init(&queue);
    int service_time[NoServiceMan]; // 서비스의 alarm clock
    ...
    while(clock < duration){
        ...
        for(int k=0; k++; k< NoServiceMan) { // every service man k
...
void insert_customer(int arrival_time){
    ...
    printf("고객 %d이 %d분에 들어옵니다. 서비스시간은 %d분입니다.", customer.id, customer.arrival_time, customer.service_time);
}
...
void print_stat(){
    ...
    printf("1인당 평군 대기 시간 = %f분", (double)waited_time/served_customers);
    printf("아직 대기중인 고객수 = %d", customers-served_customers);
}
